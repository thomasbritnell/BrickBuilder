...
void Particle3D::update(bool friction)
{
    ...
    if (this->age < Particle3D::PARTICLE_LIFESPAN)
    {
        ...
        if (this->direction.multiply(this->speed).movePoint(this->position).mY >= 0)
        {
            ...
            acceleration = Vec3D(0, -((0.049 * size) / speed), 0);//the 0.049 is mentioned in the readme. It is the gravitational constant scaled
        }
        else
        {
            //if friction, then the velocity after bouncing decreases
